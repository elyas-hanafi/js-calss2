 ...
 * Variables are containers for storing data values. In TypeScript, you can declare a variable using `let`, `const`, or `var`.
 */
let myName: string = "Alice"; // String
const myAge: number = 25; // Number
...
/**
 * Data Types:
 * TypeScript has several data types, including:
...
let nullVariable: null = null; // Null
let person: { firstName: string; lastName: string } = {
  firstName: "John",
  lastName: "Doe",
}; // Object
let hobbies: string[] = ["Reading", "Traveling", "Coding"]; // Array
 ...
 */
let sum: number = 5 + 3; // Addition
let difference: number = 10 - 6; // Subtraction
let product: number = 4 * 7; // Multiplication
let quotient: number = 20 / 4; // Division
let remainder: number = 9 % 4; // Modulus (remainder)
 ...
 */
let greeting: string = "Hello, " + myName + "!"; // Using +
let templateGreeting: string = `Hello, ${myName}! You are ${myAge} years old.`; // Using template literals
...
let convertedNumber: number = Number(stringNumber); // Type Conversion (String to Number)
console.log(convertedNumber); // Outputs: 123

let coercedResult: string = "5" + 5; // Type Coercion (5 is converted to "5", result is "55")
 ...
 */
let falsyValue: number = 0; // 0 is falsy
let truthyValue: number = 1; // 1 is truthy
 ...
 */
let canVote: string =
  myAge >= 18 ? "Yes, you can vote." : "No, you cannot vote.";
...
function calculateSquareAndAdd(a: number, b: number): number {
 ...
 * TypeScript objects allow properties to be accessed using dot notation (object.property)
 ...
 * TypeScript provides built-in methods to manipulate objects and arrays.
 */
...
// Get object keys
let keys: string[] = Object.keys(personInfo);
console.log(keys); // Outputs: ["name", "age", "city"]

// Get object values
let values: any[] = Object.values(personInfo);
console.log(values); // Outputs: ["John", 30, "New York"]

// Get object entries (key-value pairs)
let entries: [string, any][] = Object.entries(personInfo);
